    ...
    <div class="flex flex-1 min-h-0">
      ...
      <aside class="w-64 bg-bg-secondary border-r border-border flex flex-col">
        ...
        <div class="p-4 border-t border-border text-xs text-text-muted">
          <div>Nexus Desktop v{{ appVersion }}</div>
        </div>
      </aside>
...
const menuItems = [
  ...
  { path: '/query', label: 'Query Editor', icon: 'fas fa-terminal' },
  { path: '/graph', label: 'Graph View', icon: 'fas fa-project-diagram' },
  { path: '/schema', label: 'Schema', icon: 'fas fa-sitemap' },
  { path: '/data', label: 'Data Browser', icon: 'fas fa-table' },
  { path: '/indexes', label: 'Indexes', icon: 'fas fa-list-ol' },
...
const pageTitle = computed(() => {
  ...
  return titles[route.path] || 'Nexus Desktop';
});

const pageIcon = computed(() => {
  const icons: Record<string, string> = {
    '/': 'fas fa-tachometer-alt',
    '/query': 'fas fa-terminal',
    '/graph': 'fas fa-project-diagram',
    '/schema': 'fas fa-sitemap',
    '/data': 'fas fa-table',
    '/indexes': 'fas fa-list-ol',
    '/logs': 'fas fa-file-alt',
    '/config': 'fas fa-cog',
  };
  return icons[route.path] || 'fas fa-project-diagram';
});
...
onMounted(async () => {
  ...
  appVersion.value = await ipcBridge.getVersion();
