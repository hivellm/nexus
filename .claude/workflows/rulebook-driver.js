...
VERIFICATION BUDGET (hard limits — this is a large Rust workspace):
- Type-check with \`cargo check -p nexus-core\` (or the touched package) ONLY — never the full workspace, never clippy.
- Run ONLY the test files/modules directly touched by the diff (\`cargo test -p <pkg> --test <file>\` or \`-- <module>\`), never the whole suite.
- At most ~15 tool calls total. When you have enough evidence to judge, STOP investigating and emit the verdict.
...
MANDATORY: you MUST end by calling the StructuredOutput tool with your verdict — even if verification is incomplete. An unfinished investigation is returned as pass=false with the open questions listed in issues. NEVER finish without calling StructuredOutput.`,
      {
...
Re-run the type-checker and the relevant tests (both must pass). Report which files you changed.`,
      { label: `fanout-fix:${taskId}:r${fround}`, phase: 'Fanout', agentType: 'implementer', model: 'sonnet' }
    )
  }
