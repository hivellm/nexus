//! Cartesian-product application, row materialisation and update,
//! entity id / relationship-value extraction, and the context
//! expression evaluator used before operators have materialised rows.
//! Extracted from `eval/helpers.rs`.
        ...
        for row_map in rows {
            ...
            let is_duplicate = if all_entity_ids.len() > 1 {
                ...
                let row_key = match serde_json::to_string(row_map) {
                    Ok(s) => s,
                    Err(e) => {
                        super::super::super::serde_metrics::record_fallback(
                            super::super::super::serde_metrics::SerdeFallbackSite::HelperRowDedupKey,
                        );
                        tracing::warn!(
