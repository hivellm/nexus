...
- [x] 12.1 Update or create documentation covering the implementation — per-SDK READMEs rewritten for RPC-first, per-SDK CHANGELOG 1.0.0 entries rewritten to cover the transport work, `docs/specs/sdk-transport.md` carries the shared contract, and `docs/MIGRATION_SDK_TRANSPORT.md` is the new cross-SDK migration guide.
- [x] 12.2 Write tests covering the new behavior — 9 Rust integration + 11 Rust unit + 10 Rust live-gated tests (`NEXUS_SDK_LIVE_TEST=1`); 38 TypeScript Vitest tests; 44 Python pytest tests; 34 Go tests (`go test ./transport/...`); 49 C# xUnit tests; 30+ PHP PHPUnit tests.
- [x] 12.3 Run tests and confirm they pass — each SDK's native test command passes on its dev toolchain: `cargo test -p nexus-sdk` (Rust), `npx vitest run tests/transports.test.ts` (TypeScript), `pytest tests/test_transport.py` (Python), `go test ./transport/...` (Go), `dotnet test Tests/Nexus.SDK.Tests.csproj` (C#). PHP runs in CI. Aggregated: `pwsh sdks/run-all-comprehensive-tests.ps1 -Transport rpc`.
