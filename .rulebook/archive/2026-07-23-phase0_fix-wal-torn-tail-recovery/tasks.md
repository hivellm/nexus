...
- [x] 1.1 Write a failing unit/integration test on `Wal`: append several valid
...
- [x] 1.2 Add a CRC-mismatch variant: append valid entries, flush, then corrupt the
  ...
  the same way
- [x] 1.3 Add a "poison persists across reboots" test: after 1.1's truncated file,
...
- [x] 2.1 Define precisely how to detect "this CRC-mismatched frame is the last one
  ...
  DECISION: condition (a) — `file_offset + frame_len > file_len` (declared
  length runs past EOF) marks a torn frame BEFORE allocating/reading its payload
  (also caps the recovery-time allocation); for a fully-read frame that then
  fails CRC, `file_offset + frame_len >= file_len` (nothing after it) means it is
  the last frame -> truncate; otherwise hard error. A mid-file frame is always
  fully present (written whole before the next), so frame_len never exceeds the
  remaining bytes for it — the check only ever fires on the true trailing frame.
- [x] 2.2 Confirm the `TruncatedTrailing` semantics used by `decode_v3_frame`
...
## 3. Implement the fix
- [x] 3.1 In the v2 body-read arm (`writer.rs:502-514`), wrap each `read_exact`
...
- [x] 3.2 Apply the identical fix to the v1 body-read arm (`writer.rs:528-540`)
- [x] 3.3 Change the CRC-mismatch branch (`writer.rs:599-604`) to check the §2.1
...
- [x] 3.4 Make the §1.1 and §1.2 tests pass (assert `recover()` returns `Ok` with
...
- [x] 4.1 Update `docs/specs/wal-mvcc.md` with the recovery contract: a torn
...
- [x] 4.2 Tests: torn-tail EOF recovers the prefix and truncates (v1 and v2 framing
