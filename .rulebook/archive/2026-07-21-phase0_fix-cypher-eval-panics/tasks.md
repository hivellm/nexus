...
- [x] 1.1 `RETURN date('2020-01-01') + duration({days: 999999999})` — assert a Cypher error, not
  a chrono panic (`temporal.rs:194-265`, dispatched via `try_datetime_add`, `temporal.rs:54`)
- [x] 1.2 `RETURN datetime('2020-01-01T00:00:00Z') + duration({days: 100000000})` — same
  assertion for the datetime add arm (`temporal.rs:210-265`)
- [x] 1.3 A subtract-direction case, e.g.
  `RETURN date('2020-01-01') - duration({days: 999999999})` — assert a Cypher error, not a panic
  (`temporal.rs:306-361`)
- [x] 1.4 `RETURN duration({years: 9223372036854775807}) + duration({years: 1})` — assert a
  Cypher error, not a silently wrapped/wrong duration (`temporal.rs:81-86`) or a debug panic
- [x] 1.5 A duration subtraction overflow case — assert a Cypher error (`temporal.rs:147-152`)
- [x] 1.6 `WITH [1.0,2.0,3.0] AS v UNWIND v AS x RETURN percentile_cont(x, 1.5)` — assert a
  Cypher error, not an out-of-bounds panic (`aggregate/core.rs:866-877`)
- [x] 1.7 `RETURN percentile_cont(1.0, -0.5)` over a small list — confirm the current
...
- [x] 2.1 In `crates/nexus-core/src/executor/eval/temporal.rs`, replace the unchecked component
...
- [x] 2.2 Replace the chrono `Duration::seconds`/`::days` calls and the `DateTime`/`NaiveDate`/
...
- [x] 2.3 Confirm the §1.1, §1.2, §1.3 tests pass

## 3. Fix duration ± duration overflow
- [x] 3.1 In `temporal.rs:81-86` (add), replace each of the six unchecked component sums (years,
  months, days, hours, minutes, seconds) with `checked_add`, returning a Cypher error on overflow
- [x] 3.2 In `temporal.rs:147-152` (subtract), apply the same treatment with `checked_sub`
- [x] 3.3 Confirm the §1.4 and §1.5 tests pass in both debug and release-profile test runs (the
  defect's failure mode differs by profile, so both must be checked)

## 4. Fix percentile_cont index panic
- [x] 4.1 In `crates/nexus-core/src/executor/operators/aggregate/core.rs:866-877`, validate
  `percentile` is within `[0,1]` before computing `position`, returning a Cypher error otherwise
- [x] 4.2 Additionally clamp `lower_idx`/`upper_idx` with `.min(values.len()-1)` as defense in
  depth, mirroring the sibling `PercentileDisc` (:837-839)
- [x] 4.3 Confirm the §1.6 and §1.7 tests pass

## 5. Secondary: fix or delete the dead-code parallel AVG merge
- [x] 5.1 In `crates/nexus-core/src/executor/operators/aggregate/parallel.rs:286-309`, confirm
...
- [x] 5.2 If kept: change the AVG merge to accumulate `(sum, count)` per chunk and compute the
  ...
  `mod` declarations)
- [x] 5.3 Either way, add a unit test proving the weighted-mean correctness (unequal chunk
...
- [x] 6.1 Update or create documentation covering the implementation:
  `docs/specs/cypher-subset.md` gained the error contract for temporal arithmetic
  overflow, duration arithmetic overflow, and the `percentileCont` argument
  domain; CHANGELOG entry added.
- [x] 6.2 Write tests covering the new behavior: all §1 regression tests pass, plus
  a boundary test per site (largest in-range value succeeds, one past it errors),
  in both debug and release.
- [x] 6.3 Run tests and confirm they pass: `cargo +nightly fmt --all`,
