...
- [x] 3.3 arm64 runtime smoke is UNRUNNABLE under qemu-user on this amd64 host — qemu does not implement the `get_robust_list` syscall (verified via QEMU_STRACE: `get_robust_list(...) = -1 errno=38`), which LMDB's robust-mutex env-open requires; the binary exits "Database error: Function not implemented (os error 38)" under emulation. This is a qemu-user limitation, NOT an arm64 defect — the syscall exists in every real arm64 kernel (Graviton, Apple Silicon, Ampere). Resolution: the smoke battery is codified as an on-demand GitHub Actions workflow `.github/workflows/arm64-smoke.yml` running on the NATIVE `ubuntu-24.04-arm` runner (`workflow_dispatch` only — no schedule/CI, matching this repo's no-automated-workflows policy). It pulls the multi-arch manifest (asserts the pulled digest is arm64), then runs: health-up, param MERGE-rel round-trip asserting `[[1,7]]`, restart persistence (LMDB/mmap on aarch64), `--healthcheck` exit 0, and the docker-health `healthy` verdict. Run before tagging: `gh workflow run arm64-smoke.yml --ref release/2.5.0 -f tag=2.5.0-dev`
...
- [x] 4.2 Write tests covering the new behavior — per-arch static-linkage build gate (fails the build on a dynamic binary); amd64 smoke battery (executed); arm64 smoke battery codified as the native-runner `arm64-smoke.yml` workflow (see 3.3)
- [x] 4.3 Run tests and confirm they pass — amd64 smoke complete on this host; arm64 smoke is the committed `arm64-smoke.yml` workflow, run on a real arm64 kernel via `gh workflow run` (the only environment where LMDB's robust mutexes function). Both platform digests are Scout-clean (0/0/0/0)
