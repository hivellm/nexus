...
**SCOPE (2026-07-23):** #4 and #5 delivered in this task. #6 (checkpoint/
truncate in production) was SPLIT OUT into `phase0_fix-wal-checkpoint-truncate-
production` because its recovery-from-checkpoint contract is sequenced AFTER
`phase0_fix-wal-torn-tail-recovery` (user delegated the scoping; recommended
option taken to avoid rework). The #6 items below are marked done-as-moved.

## 1. Reproduce and scope each gap
- [x] 1.1 Gap #4: write a test that forces `flush_batch`'s retry loop to exhaust
  ...
  DONE (inspection-verified per §1.1's allowance; not run as a live repro to
  avoid polluting CWD with a `data/` dir). Fixed behaviour pinned by
  wal/mod.rs::emergency_* tests + async_wal.rs::async_emergency_save_is_recoverable.
- [x] 1.2 Gap #5: confirm by inspection that `Wal::new` (`writer.rs:132-163`),
  ...
  DECISION: inspection-verified. Windows (this env) has no std directory-fsync,
  so the helper is a documented no-op there; the Unix path does the real fsync.
  Helper: storage/fs.rs::sync_parent_dir (best-effort, cfg(unix) real / no-op).
- [x] 1.3 Gap #6: MOVED to phase0_fix-wal-checkpoint-truncate-production. `grep -rn "\.checkpoint(\|\.truncate()" crates/nexus-core/src` and
...
- [x] 2.1 Gap #4: decide whether the emergency batch is written through the real
  ...
  DECISION: reuse the real machinery — new `Wal::emergency_save` opens a second
  `Wal` at `{wal_dir}/wal-emergency-{ts}.log` mirroring the main WAL's cipher and
  calls `append()` (byte-identical frames; no plaintext leak when encrypted).
  Boot scan: `Wal::recover_emergency` globs `wal-emergency-*.log` in the WAL dir,
  decodes with the WAL's cipher, and `recover_external_ids_from_wal` merges those
  entries AFTER the main WAL's (put_if_absent is idempotent, so order is
  irrelevant), then `clear_emergency` removes the files once durably applied.
- [x] 2.2 Gap #5: decide the directory-fsync helper's shape (a small shared
  ...
  DECISION: `pub(crate) storage::fs::sync_parent_dir(path)` returns `()`,
  best-effort (logs + continues on failure), `#[cfg(unix)]` real / no-op elsewhere.
- [x] 2.3 Gap #6: MOVED to phase0_fix-wal-checkpoint-truncate-production. decide the checkpoint trigger policy (entry count threshold,
...
- [x] 3.1 (Gap #4) Reroute `emergency_save_batch` (`async_wal.rs:503-526`) to write
...
- [x] 3.2 (Gap #4) Make the §1.1 test pass: after the fix, entries that hit the
  emergency-save path are present in `recover()`'s output following a normal
  restart-and-recover sequence
- [x] 3.3 (Gap #5) Add the directory-fsync helper from §2.2; call it from
...
- [x] 3.4 (Gap #5) Make the §1.2 verification pass (test or documented inspection
  confirmation, per what §1.2 established as feasible)
- [x] 3.5 (Gap #6) MOVED to phase0_fix-wal-checkpoint-truncate-production. Wire the periodic checkpoint per §2.3's trigger policy into the
...
- [x] 3.6 (Gap #6) MOVED to phase0_fix-wal-checkpoint-truncate-production. Make the §1.3 test pass: a long-running append sequence now
...
- [x] 4.1 Update `docs/specs/wal-mvcc.md` with the emergency-save real-frame
  replay contract and the directory-fsync guarantee for newly created WAL and
  record-store files (checkpoint/truncate contract moved to the #6 follow-up).
  Add a CHANGELOG entry covering #4 and #5
- [x] 4.2 Tests: emergency-saved entries survive save+recover (unit + async
  fail_flush end-to-end); cipher-mirrored emergency file does not leak plaintext;
  directory-fsync helper exercised (storage::fs tests). Checkpoint/truncate tests
  moved to the #6 follow-up
