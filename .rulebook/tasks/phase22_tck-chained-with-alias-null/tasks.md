## 1. Implementation
- [ ] 1.1 Reproduce all five variants above as a failing test, then root-cause where the second `WITH` loses the alias binding (name the file:line before editing)
- [ ] 1.2 Fix alias resolution so a `WITH`-minted alias binds for every downstream clause irrespective of its source clause
- [ ] 1.3 Verify the three-`WITH` case and the `UNWIND` + `MATCH` mixed case do not regress
