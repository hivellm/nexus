...
## 1. Implementation
- [x] 1.1 Commit a repeatable relationship-ingest micro-benchmark (extend the
...
- [x] 1.2 Add `Catalog::batch_increment_rel_counts(&[(TypeId, u32)])` in
  ...
  count-equivalence.
- [x] 1.3 Stop the per-edge increment on the `/ingest` path: remove the
...
- [x] 1.4 Re-measure with §1.1. Gate: relationship ingest ≥ node ingest
...
- [x] 1.5 Close the executor CREATE path's rel-count gap (`engine/mod.rs:481`
  ...
  AND `UNWIND … CREATE`.
- [x] 1.6 Measure the isolated cost of the redundant Phase-8 per-edge write
...
- [x] 2.1 Update or create documentation covering the implementation — refresh
...
- [x] 2.2 Write tests covering the new behavior — `batch_increment_rel_counts`
...
- [x] 2.3 Run tests and confirm they pass — `cargo +nightly fmt --all`,
