...
- [x] 2.1 Module-level doc on `AsyncWalStats` calls out the atomic-counter / snapshot-view model and references the pre-fix race (casting `Arc<AsyncWalStats>` through `*mut` aliased `&mut` across threads under the Rust memory model)
- [x] 2.2 Regression test `concurrent_appends_count_exactly` drives 10 concurrent `append()` calls and asserts `entries_submitted == 10` — with the old pointer-cast implementation this count could come in below 10 under Miri / stressed loads
- [x] 2.3 `cargo +nightly test -p nexus-core --lib wal` → 26 passed, 0 failed (26 = 25 previous + 1 new concurrent test)
