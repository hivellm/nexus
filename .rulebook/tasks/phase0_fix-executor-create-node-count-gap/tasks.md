...
## 1. Implementation
- [ ] 1.1 In `execute_create_with_context`, add a
...
- [ ] 1.2 After the transaction commit, flush once via
...
- [ ] 2.1 Update the `engine/mod.rs` count docs / any stats spec if they still
  imply this path under-counts nodes.
- [ ] 2.2 Test: after `UNWIND range(1, K) AS i CREATE (n:Label)` the catalog node
...
- [ ] 2.3 Run tests — `cargo +nightly fmt --all`,
