## 1. Implementation
- [ ] 1.1 Parse a bare pattern as a predicate in WHERE, routing it to the evaluation path the `NOT` form already uses
- [ ] 1.2 Parse the function-style `exists(pattern)` to the same node as `exists { pattern }`
- [ ] 1.3 Cover composition with AND/OR/XOR/NOT and with a relationship type, direction, and inline properties
