...
- [ ] 2.1 Implement real side-effect assertions. **Measured against the vendored corpus: 1498 of 1615 scenarios (93%) assert side effects** — 1180 use `And no side effects`, 244 use `And the side effects should be:` with tables like `| +nodes | 1 |`. Skipping them is therefore not an option; it would gut the corpus. The counters the TCK expects are `+nodes`/`-nodes`, `+relationships`/`-relationships`, `+properties`/`-properties`, `+labels`/`-labels`
  ...
  **Scope correction (2026-07-25):** the "counters do not exist" note above is now STALE. The scaffolding was since added: `SideEffects` (8 counters, TCK vocabulary) is defined at `executor/types.rs:161-185` and `ResultSet.side_effects` at `:204`, with a dedicated test `tests/executor/side_effects.rs`. Mechanism: a per-query accumulator — node/rel creation is counted via an `Arc<AtomicU64>` chokepoint on `RecordStore` (shared across `refresh_executor` clones), reset+read in `engine/query_pipeline.rs` (both entry points); other counters are meant to be incremented on the engine's `self.side_effects`. **Wired so far (4/8, all tested in `tests/executor/side_effects.rs`): `nodes_created` (pre-existing), `relationships_created` (`create_relationship` storage chokepoint), `nodes_deleted` + `relationships_deleted` (engine-level: `self.side_effects` incremented in `crud/nodes.rs` `delete_node`/`delete_relationship`/`delete_node_relationships`, real-deletion path only, no double-count on `DELETE r, n` DETACH).** STILL TODO (4/8): `properties_set`, `properties_removed`, `labels_added`, `labels_removed`; the `/cypher` `stats` field; the TCK harness assertion (still a no-op, intertwined with §3.2's runner generalisation).
  **Semantic decision for the remaining 4 — resolve before implementing:** per openCypher TCK / Neo4j, `CREATE (n:L {p:1})` reports `+labels 1` AND `+properties 1` — i.e. labels/properties on CREATE-d nodes COUNT, not only `SET`/`REMOVE`. This CONFLICTS with the current `types.rs` `labels_added` doc ("Labels added to an existing node (SET n:Label)") and with the existing CREATE tests that assert `labels_added=0`/`properties_set=0`. Correct TCK-conformant impl must count inline CREATE labels/properties too (touching the create path again) AND `SET n:Label` / `SET n.k=v` / `SET n=/+={map}` / `REMOVE n.k` / `REMOVE n:Label` / `SET n.k=null` (a remove) — then update those CREATE test expectations to the Neo4j-correct non-zero values and fix the `types.rs` doc. Property writes likely funnel through `property_store::store_properties` (a candidate chokepoint, counting map keys) but single-key `SET n.k` vs whole-map `SET n=` granularity needs verifying first.
