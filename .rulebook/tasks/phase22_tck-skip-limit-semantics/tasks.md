## 1. Implementation
- [ ] 1.1 `LIMIT 0` returns zero rows
- [ ] 1.2 Resolve parameters in SKIP and LIMIT
- [ ] 1.3 Raise the spec's error kind for float, negative, and non-constant SKIP/LIMIT arguments
- [ ] 1.4 Apply a `WITH ... SKIP/LIMIT` before the next clause consumes rows, including an aggregating one
- [ ] 1.5 Re-measure the with-orderBy rows against the grouping-key fix and record what belonged to which cause
