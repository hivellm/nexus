## 1. Implementation
- [ ] 1.1 Honour `date:`, `time:`, and `datetime:` map components as value sources in all five constructors
- [ ] 1.2 Let scalar components override the selected fields
- [ ] 1.3 Implement the single-argument cross-type form (`datetime(localdatetime)`, `localdatetime(datetime)`, ...) with spec truncation/widening
- [ ] 1.4 Default absent fields per spec (epoch date / midnight), never to the current clock
