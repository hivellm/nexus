...
- [ ] 1.1 G1: a server test that creates database `alpha`, writes a node under it,
  ...
  not rediscovered).
- [ ] 1.2 G2: a test asserting the default is a single store — a node created via the
...
- [ ] 1.3 G3: a test that a non-Admin caller cannot `CREATE DATABASE` / `POST
...
- [ ] 2.1 Make `server.engine` and the `DatabaseManager`'s default database be the
  ...
  default store.
- [ ] 2.2 Confirm `SHOW DATABASES` / `list_databases` / `GET /databases` now report
...
- [ ] 3.1 On `DatabaseManager` construction (or a startup hook wired in `main.rs`),
...
- [ ] 3.2 Define and document what qualifies as a database directory (e.g. contains a
...
- [ ] 3.3 Make `create_database` behave sanely when the dir already exists (re-adopt
  vs error) now that discovery runs — no accidental reinit of a populated dir.

## 4. G3 — authenticate database management
- [ ] 4.1 Gate `CREATE DATABASE` / `DROP DATABASE` (Cypher `execute_database_commands`)
...
- [ ] 4.2 Preserve the auth-disabled bootstrap path (no identity present -> no-op),
...
- [ ] 5.2 Write tests covering the new behavior (the §1 tests now passing, plus a
  drop-then-recreate-after-restart case and a stretch test for `/cypher/stream`
  routing if that optional item is taken).
- [ ] 5.3 Run tests and confirm they pass (`cargo +nightly fmt --all`,
