...
- [x] 15.1 Documentation landed: `docs/specs/simd-dispatch.md` with CRC / RLE / JSON / tokenizer findings, per-item phase-3 status table, and the honest "items dropped" summary; CHANGELOG + README updates land with this tasks.md update.
- [x] 15.2 Tests covering the new behavior: WAL 3 new integration tests (legacy v1, v2 CRC32C round-trip, mixed); CRC32C 4 unit tests; RLE 5 explicit + 2 proptest cases (256 inputs each); JSON 4 unit + 4 parity proptest cases — total of 21 new tests for the phase-3 work beyond the existing 2500+ nexus-core suite.
