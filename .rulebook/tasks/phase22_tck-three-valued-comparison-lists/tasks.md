## 1. Implementation
- [ ] 1.1 Propagate null from element-wise list equality/inequality, recursively for nested lists
- [ ] 1.2 Apply the same Kleene rules to `IN` over a list containing null
- [ ] 1.3 Confirm WHERE-filter truthiness is unchanged (null still drops the row) — only the operators become strict
