...
- [ ] 1.1 Decide: (a) wire a minimal KNN write-path maintenance hook now (a `knn_evict_node` call
...
- [ ] 1.2 If (b) is chosen, confirm the mapping-bug fix is still fully testable via direct calls to
...
- [ ] 2.1 Write a failing test: `add_vector(id, v1)`, then `add_vector(id, v2)` (same id, same
  ...
  broad-radius query
- [ ] 2.2 Write a failing test: `add_vector(id, v1)`, `add_vector(id, v2)` (triggering the §2.1
  leak), then `remove_vector(id)`. Confirm the orphaned v1 HNSW entry is STILL reachable after
  removal — a phantom hit after "delete"
- [ ] 2.3 Confirm via code inspection that `add_vector`'s existing-id branch
...
- [ ] 3.1 Change `add_vector` so that when `id` already has an entry, it removes the old HNSW
...
- [ ] 3.2 Make the §2.1 test pass: after re-insert, only the current vector's entry is reachable
- [ ] 3.3 Make the §2.2 test pass: after re-insert then remove, no entry for `id` (old or new) is
  reachable

## 4. Apply the §1 wiring decision
- [ ] 4.1 If (a) was chosen in §1.1: implement `knn_evict_node` in `engine/crud/` and call it from
...
- [ ] 4.2 If (b) was chosen: implement `knn_evict_node` as a standalone function callable the same
  ...
  task
- [ ] 4.3 Test `knn_evict_node` directly: add a vector, evict, confirm both `node_to_index` and
  `index_to_node` no longer reference the node

## 5. Tail (docs + tests — check or waive with tailWaiver)
- [ ] 5.1 Update `docs/specs/knn-integration.md` with the vector re-insert/evict contract (single
  HNSW entry per node id, invariant maintained across update and delete); add a CHANGELOG entry
  noting the §1 scope decision
- [ ] 5.2 Tests: re-insert does not leak the old entry (§2.1/§3.2), remove after re-insert leaves
