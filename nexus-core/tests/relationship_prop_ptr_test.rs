...
fn test_create_relationship_preserves_source_node_prop_ptr() -> Result<()> {
    ...
    assert!(source_properties.is_some(), "Source node properties should be accessible");
...
fn test_create_relationship_preserves_target_node_prop_ptr() -> Result<()> {
    ...
    assert!(target_properties.is_some(), "Target node properties should be accessible");
...
fn test_create_multiple_relationships_preserves_prop_ptr() -> Result<()> {
    ...
    // Create target nodes
    let target1_id = store.create_node_with_label_bits(
        &mut tx,
        2,
        serde_json::json!({"name": "Acme"}),
    )?;
    
    let target2_id = store.create_node_with_label_bits(
        &mut tx,
        2,
        serde_json::json!({"name": "TechCorp"}),
    )?;
    ...
    assert_eq!(
        source_node_after_rel1.prop_ptr,
        original_prop_ptr,
    ...
    assert_eq!(
        source_node_after_rel2.prop_ptr,
        original_prop_ptr,
...
fn test_prop_ptr_validation_on_write() -> Result<()> {
    ...
    // Create a node
    let node_id = store.create_node_with_label_bits(
        &mut tx,
        1,
    ...
    assert!(result.is_err(), "write_node should reject corrupted prop_ptr");
