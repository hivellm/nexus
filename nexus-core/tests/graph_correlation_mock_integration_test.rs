...
fn test_call_graph_with_mock_vectorizer() {
    ...
    mock.add_function("process_data", "lib.rs", "fn process_data(input: String) -> Result<()>");
    ...
    source_data.add_file("main.rs".to_string(), "fn main() { process_data(); }".to_string());
    source_data.add_file("lib.rs".to_string(), "fn process_data() { validate(); }".to_string());
    
    source_data.add_functions("main.rs".to_string(), vec!["main".to_string()]);
    source_data.add_functions("lib.rs".to_string(), vec!["process_data".to_string(), "validate".to_string()]);
...
fn test_pattern_detection_with_mock_data() {
    ...
    source_data.add_functions(
        ...
        vec!["stage1".to_string(), "stage2".to_string(), "stage3".to_string()],
    );
