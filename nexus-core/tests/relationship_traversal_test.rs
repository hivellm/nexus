...
fn test_relationship_linked_list_traversal() {
    ...
    let node1 = store.create_node(&mut tx, vec![], serde_json::json!({})).unwrap();
    let node2 = store.create_node(&mut tx, vec![], serde_json::json!({})).unwrap();
    let node3 = store.create_node(&mut tx, vec![], serde_json::json!({})).unwrap();
    ...
    let rel1 = store.create_relationship(&mut tx1, node1, node2, 1, serde_json::json!({})).unwrap();
    ...
    let rel2 = store.create_relationship(&mut tx2, node1, node3, 1, serde_json::json!({})).unwrap();
    ...
    while ptr != 0 {
        ...
        if count > 10 { break; } // Safety break
    }
