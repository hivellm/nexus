...
fn test_union_basic() -> Result<(), Error> {
    ...
    assert_eq!(
        result.rows.len(),
        5,
        "Should have 5 unique names: 3 people + 2 companies"
    );
...
fn test_union_vs_union_all_duplicates() -> Result<(), Error> {
    ...
    let union_result = engine
        .execute_cypher("MATCH (x:X) RETURN x.val AS v UNION MATCH (y:Y) RETURN y.val AS v")?;

    let union_all_result = engine
        .execute_cypher("MATCH (x:X) RETURN x.val AS v UNION ALL MATCH (y:Y) RETURN y.val AS v")?;

    assert_eq!(
        union_result.rows.len(),
        3,
        "UNION should deduplicate: 10, 20, 30"
    );
    assert_eq!(
        union_all_result.rows.len(),
        4,
        "UNION ALL should keep all: 10, 20, 10, 30"
    );
...
fn test_union_empty_left() -> Result<(), Error> {
    ...
    assert_eq!(
        result.rows.len(),
        3,
        "Should return only right side results"
    );
...
fn test_union_three_way() -> Result<(), Error> {
    ...
    assert_eq!(
        result.rows.len(),
        8,
        "Should have 3 people + 2 companies + 3 products"
    );
...
fn test_union_multiple_columns() -> Result<(), Error> {
    ...
    assert_eq!(
        result.rows.len(),
        3,
        "Should deduplicate (Alice, 30): Bob, Alice, Charlie"
    );
...
fn test_union_identical_queries() -> Result<(), Error> {
    ...
    assert_eq!(
        result.rows.len(),
        3,
        "Identical queries should deduplicate to original count"
    );
...
fn test_union_mixed_types() -> Result<(), Error> {
    ...
    assert_eq!(
        result.rows.len(),
        2,
        "Should deduplicate: 'text', 'different'"
    );
...
fn test_union_order_independence() -> Result<(), Error> {
    ...
    assert_eq!(
        result1.rows.len(),
        result2.rows.len(),
        "Order should not matter for UNION"
    );
...
fn test_union_with_nulls() -> Result<(), Error> {
    ...
    assert!(
        ...
        "Should handle null values in UNION"
    );
