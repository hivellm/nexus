...
fn setup_test_data(engine: &mut Engine) -> Result<(), Error> {
    ...
    engine.execute_cypher(
        "CREATE (p:Product {name: 'Laptop', price: 1000, category: 'Electronics'})",
    )?;
    engine
        .execute_cypher("CREATE (p:Product {name: 'Mouse', price: 25, category: 'Electronics'})")?;
    engine.execute_cypher(
        "CREATE (p:Product {name: 'Keyboard', price: 75, category: 'Electronics'})",
    )?;
    engine.execute_cypher(
        "CREATE (p:Product {name: 'Monitor', price: 300, category: 'Electronics'})",
    )?;
    engine
        .execute_cypher("CREATE (p:Product {name: 'Desk', price: 300, category: 'Furniture'})")?;
    engine
        .execute_cypher("CREATE (p:Product {name: 'Chair', price: 150, category: 'Furniture'})")?;
...
fn test_count_distinct_vs_regular_count() -> Result<(), Error> {
    ...
    assert!(
        distinct_count < regular_count,
        "DISTINCT should be less than regular count"
    );
...
fn test_count_distinct_multiple_labels() -> Result<(), Error> {
    ...
    let result = engine.execute_cypher(
        "MATCH (p:Person:Employee) RETURN count(DISTINCT p.dept) AS unique_depts",
    )?;
    let count = result.rows[0].values[0].as_i64().unwrap();

    assert_eq!(
        count, 2,
        "Should have 2 distinct departments: Engineering, Sales"
    );
...
fn test_count_distinct_products_price() -> Result<(), Error> {
    ...
    assert_eq!(
        count, 5,
        "Should have 5 distinct prices: 25, 75, 150, 300, 1000"
    );
...
fn test_count_distinct_all_same_value() -> Result<(), Error> {
    ...
    let result =
        engine.execute_cypher("MATCH (i:Item) RETURN count(DISTINCT i.type) AS unique_types")?;
    let count = result.rows[0].values[0].as_i64().unwrap();
...
fn test_count_distinct_with_null_values() -> Result<(), Error> {
    ...
    let result =
        engine.execute_cypher("MATCH (n:Node) RETURN count(DISTINCT n.value) AS unique_values")?;
    let count = result.rows[0].values[0].as_i64().unwrap();
...
fn test_count_distinct_string_values() -> Result<(), Error> {
    ...
    let result =
        engine.execute_cypher("MATCH (u:User) RETURN count(DISTINCT u.name) AS unique_names")?;
    let count = result.rows[0].values[0].as_i64().unwrap();

    assert_eq!(
        count, 4,
        "Should have 4 distinct names: Alice, Bob, Charlie, David"
    );
...
fn test_count_distinct_large_dataset() -> Result<(), Error> {
    ...
    let result = engine
        .execute_cypher("MATCH (n:BigData) RETURN count(DISTINCT n.value) AS distinct_values")?;
    let count = result.rows[0].values[0].as_i64().unwrap();
...
fn test_count_distinct_with_limit() -> Result<(), Error> {
    ...
    let result =
        engine.execute_cypher("MATCH (p:Person) RETURN count(DISTINCT p.age) AS ages LIMIT 1")?;
    let count = result.rows[0].values[0].as_i64().unwrap();
