...
fn test_count_distinct_deduplicates() -> Result<(), Error> {
    ...
    assert_eq!(count, 2, "COUNT(DISTINCT) should return unique non-NULL count");
    Ok(())
}
...
fn test_min_max_ignore_nulls() -> Result<(), Error> {
    ...
    let min = min_result.rows[0][0].as_i64().unwrap();
    
    let max_result = engine.execute_cypher("MATCH (n:Node) RETURN max(n.value) AS max")?;
    let max = max_result.rows[0][0].as_i64().unwrap();
...
fn test_union_removes_duplicates() -> Result<(), Error> {
    ...
    let result = engine.execute_cypher(
        "MATCH (a:A) RETURN a.value AS v UNION MATCH (b:B) RETURN b.value AS v"
    )?;
...
fn test_union_requires_same_column_count() -> Result<(), Error> {
    ...
    let result = engine.execute_cypher(
        "MATCH (a:A) RETURN a.a AS col UNION MATCH (b:B) RETURN b.x AS col"
    )?;
    
    assert!(result.rows.len() >= 0, "UNION with same column count should succeed");
    Ok(())
}
...
fn test_relationship_direction_matters() -> Result<(), Error> {
    ...
    let alice = engine.create_node(vec!["Person".to_string()], serde_json::json!({"name": "Alice"}))?;
    let bob = engine.create_node(vec!["Person".to_string()], serde_json::json!({"name": "Bob"}))?;
    
    engine.create_relationship(alice, bob, "KNOWS", serde_json::json!({}))?;
    engine.refresh_executor()?;
    
    // Outgoing from Alice
    let outgoing = engine.execute_cypher("MATCH (a {name: 'Alice'})-[:KNOWS]->(b) RETURN count(*) AS count")?;
    let out_count = outgoing.rows[0][0].as_i64().unwrap();
    
    // Incoming to Alice  
    let incoming = engine.execute_cypher("MATCH (a {name: 'Alice'})<-[:KNOWS]-(b) RETURN count(*) AS count")?;
    let in_count = incoming.rows[0][0].as_i64().unwrap();
...
fn test_bidirectional_pattern_counts_both() -> Result<(), Error> {
    ...
    let result = engine.execute_cypher("MATCH (a {name: 'Alice'})-[:KNOWS]-(b) RETURN count(*) AS count")?;
    let count = result.rows[0][0].as_i64().unwrap();
...
fn test_labels_returns_array() -> Result<(), Error> {
    ...
    assert!(result.rows[0][0].is_array(), "labels() should return array");
...
fn test_keys_returns_property_names() -> Result<(), Error> {
    ...
    assert!(result.rows[0][0].is_array(), "keys() should return array");
...
fn test_id_returns_unique_identifier() -> Result<(), Error> {
    ...
    let id1 = result.rows[0][0].as_i64();
    let id2 = result.rows[1][0].as_i64();
...
fn test_where_property_equals() -> Result<(), Error> {
    ...
    let result = engine.execute_cypher("MATCH (n:Person) WHERE n.age = 30 RETURN count(*) AS count")?;
    let count = result.rows[0][0].as_i64().unwrap();
...
fn test_where_null_check() -> Result<(), Error> {
    ...
    let result = engine.execute_cypher("MATCH (n:Node) WHERE n.value IS NOT NULL RETURN count(*) AS count")?;
    let count = result.rows[0][0].as_i64().unwrap();
...
fn test_aggregation_on_empty_returns_null() -> Result<(), Error> {
    ...
    let avg = &avg_result.rows[0][0];
    
    assert!(avg.is_null() || avg.as_f64().unwrap_or(0.0) == 0.0, "AVG on empty should be NULL or 0");
    Ok(())
}
