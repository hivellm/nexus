...
> **Status**: Three separate metrics, none of them a substitute for another —
> **Neo4j differential suite 308/325** (2 failed, 15 skipped; against a live Neo4j
> 2025.09.0, both failures the documented MERGE write-path gaps 15.08/15.12);
> **openCypher TCK conformance 48.9%** (1892 passed / 1905 failed / 71 skipped of
> 3868 outline-expanded scenarios — the strict-spec number, and the one to quote
> when someone asks "is it Cypher-compatible"); workspace
> `cargo +nightly test --workspace --no-fail-fast` reports **5829 passed / 3 failed**
> (the 3 are documented flakes that pass in isolation — see the flaky-test notes).
> Production-readiness statement intentionally dropped from this header — see the
> "Production readiness" criterion section below for the maintainers' explicit gate
> (auth + persistence + compat + RPC transport).
...
- **Neo4j Compatibility**: 308/325 on the Neo4j differential suite (curated queries; 2 failed, 15 skipped — the 2 are the documented MERGE write-path gaps); openCypher TCK conformance 48.9% (strict spec, 1892/3868 scenarios; see docs/compatibility/OPENCYPHER_TCK_REPORT.md)
...
- **Test surface**: 5829 workspace tests passing on `cargo +nightly test --workspace --no-fail-fast` (3 failed — documented flakes that pass in isolation); **308/325 on the Neo4j differential suite** (see MEASUREMENT clarification below). Re-run the numbers via `scripts/compatibility/test-neo4j-nexus-compatibility-200.ps1` (diff — needs a live Neo4j on :7474 AND Nexus on :15474; `docker run -d -p 7474:7474 -p 7687:7687 -e NEO4J_AUTH=neo4j/password neo4j:2025.09.0` provisions the Neo4j half) and `cargo +nightly test --workspace --no-fail-fast` (unit + integration; a plain `--workspace` aborts at the first failing target and silently skips the rest). See [docs/compatibility/NEO4J_COMPATIBILITY_REPORT.md](docs/compatibility/NEO4J_COMPATIBILITY_REPORT.md) for the canonical feature-by-feature status, and [docs/compatibility/OPENCYPHER_TCK_REPORT.md](docs/compatibility/OPENCYPHER_TCK_REPORT.md) for TCK conformance — that report is regenerated by the harness, so it is always the authority over any number quoted here.
