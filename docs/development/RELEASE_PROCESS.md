...
|---------|-----------------|---------------|
| Workspace crate version | `Cargo.toml` `[workspace.package].version` | `2.5.0` |
| `nexus-server` Docker image | server release workflow → mirrors workspace | `2.5.0` |
| Rust SDK (`nexus-sdk` on crates.io) | `sdks/rust/Cargo.toml` | `2.5.0` |
| Python SDK (`hivehub-nexus-sdk` on PyPI) | `sdks/python/pyproject.toml` | `2.5.0` |
| TypeScript SDK (`@hivehub/nexus-sdk` on npm) | `sdks/typescript/package.json` | `2.5.0` |
| Go SDK (`github.com/hivellm/nexus-go`) | `sdks/go/go.mod` + git tag | `2.5.0` |
| C# SDK (`Nexus.SDK` on NuGet) | `sdks/csharp/Nexus.SDK.csproj` | `2.5.0` |
| PHP SDK (`hivellm/nexus-php` on Packagist) | `sdks/php/composer.json` | `2.5.0` |
