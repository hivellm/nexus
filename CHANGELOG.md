...
### Fixed — WHERE clause parentheses now preserve operator precedence

- **WHERE clauses with parenthesized expressions now evaluate with the operator precedence the user specified instead of re-evaluating with default precedence.** A query like `MATCH (n) WHERE (n.a OR n.b) AND n.c` was silently re-parsed during execution as `n.a OR (n.b AND n.c)`, flipping the boolean result and returning incorrect rows. The root cause: WHERE predicates are lowered to a serialized string and re-parsed at execution time, and the serializer omitted parentheses from compound expressions — so the user's explicit precedence override was lost. Similarly affected parenthesized arithmetic like `(n.x - n.y) * n.z`. Fixed by making the WHERE-predicate serializer wrap every compound operand in parentheses before re-parsing, so the string round-trips to exactly the tree the user wrote. Projections (RETURN/WITH expressions) were never affected — they carry the parsed AST directly and always evaluated correctly. Not a breaking change — queries that previously returned wrong results now return correct ones.
