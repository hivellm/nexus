...
fn test_explain_create_query() {
    ...
    let check_result = engine.execute_cypher("MATCH (n:Person {name: 'Test'}) RETURN n").unwrap();
...
fn test_profile_create_query() {
    ...
    let check_result = engine.execute_cypher("MATCH (n:Person {name: 'ProfileTest'}) RETURN n").unwrap();
