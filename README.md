...
![Status](https://img.shields.io/badge/status-v3.0.0-success.svg)
...
[Quick Start](#-quick-start) • [Transports](#-transports) • [Documentation](#-documentation) • [SDKs](#-official-sdks) • [Contributing](#-contributing)
...
- **Neo4j-compatible Cypher** — MATCH / CREATE / MERGE / SET / DELETE / REMOVE / WHERE / RETURN / ORDER BY / LIMIT / SKIP / UNION / WITH / UNWIND / FOREACH / CASE / EXISTS subqueries / list & map comprehensions / pattern comprehensions and 250+ functions & procedures. **310/325** Neo4j diff-suite tests pass — 0 failed, 15 skipped ([Neo4j 2025.09.0, 2026-08-05](docs/compatibility/NEO4J_COMPATIBILITY_REPORT.md)); openCypher TCK conformance is a separate, stricter metric at **74.3%** ([2874/3868, 2026-08-15](docs/compatibility/OPENCYPHER_TCK_REPORT.md)). See the [openCypher status table](#-opencypher-support-matrix) at the bottom.
...
Six first-party SDKs, all tracking the same 3.0.0 line. Every SDK shares the URL grammar, command-map table, and error semantics defined in [`docs/specs/sdk-transport.md`](docs/specs/sdk-transport.md).

| SDK            | Install                                    | Docs                                          | RPC status   |
|----------------|--------------------------------------------|-----------------------------------------------|--------------|
| 🦀 Rust        | `nexus-sdk = "3.0.0"`                     | [sdks/rust/](sdks/rust/README.md)             | ✅ shipped   |
...
### Compatibility
- [`docs/compatibility/NEO4J_COMPATIBILITY_REPORT.md`](docs/compatibility/NEO4J_COMPATIBILITY_REPORT.md) — **310/325** tests (Neo4j 2025.09.0 diff; 0 failed, 15 skipped), per-feature breakdown.
- [`docs/compatibility/OPENCYPHER_TCK_REPORT.md`](docs/compatibility/OPENCYPHER_TCK_REPORT.md) — **74.3%** openCypher TCK conformance (2874/3868), regenerated by the harness and therefore the authority over any figure quoted by hand.
...
Canonical list of openCypher / Cypher 25 surfaces in Nexus v3.0.0. ✅ shipped · 🟡 partial (grammar-only or limited scope) · 🧭 queued. Validated against the [310/325 Neo4j diff suite](docs/compatibility/NEO4J_COMPATIBILITY_REPORT.md); per-clause detail lives in [`docs/specs/cypher-subset.md`](docs/specs/cypher-subset.md).
...
| `ORDER BY` / `SKIP` / `LIMIT` | ✅ | Top-k merge for distributed queries. Counts accept an integer literal or `$param`; a negative, non-integer or row-dependent count is rejected with the spec's error kind. A tail on a `WITH` cuts the stream there, so the next clause — an aggregation included — sees only the surviving rows. `ORDER BY` after a `WITH` is scope-checked (`InvalidAggregation` / `UndefinedVariable`). |
| `UNION` / `UNION ALL` | ✅ | |
...
| Temporal — `date()`, `datetime()`, `time()`, `duration()`, `localdatetime()` | ✅ | Map constructors accept all four calendar notations — calendar, ISO week (`{year, week, dayOfWeek}`, counted in the *week-year*), quarter, ordinal — and select from an existing value via a `date` / `time` / `datetime` key, which the map's own components override. |
