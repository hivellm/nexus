#!/usr/bin/env python3
"""Cross-SDK interop matrix for the Thunder-based Nexus RPC transport.
...
suite can: that a Thunder-based Nexus server and every Nexus client still agree
on the wire, including a pre-Thunder client still in the wild (the ``legacy``
cell).
...
Usage::
    ...
    python scripts/interop/run-matrix.py --server 127.0.0.1:25475   # external
"""
...
STEPS = ["auth", "cypher", "knn_bytes", "error"]

STEP_TITLES = {
    "auth": "authenticate",
    "cypher": "CREATE/MATCH",
    "knn_bytes": "f32-LE bytes",
...
def _from_config(pattern: str) -> str:
    ...
    The credentials and the ports have to agree between the server and every
...
REST_ADDR = _from_config(r'^server:\n  addr:\s*"([^"]+)"')
RPC_ADDR = _from_config(r'^rpc:(?:\n.*)*?\n  addr:\s*"([^"]+)"')
RESP3_ADDR = _from_config(r'^resp3:(?:\n.*)*?\n  addr:\s*"([^"]+)"')
USER = _from_config(r'^    username:\s*"([^"]+)"')
PASSWORD = _from_config(r'^    password:\s*"([^"]+)"')
RPC_PORT = int(RPC_ADDR.rsplit(":", 1)[1])
...
def discover_cells() -> list[Cell]:
    ...
    return [
        Cell(
            ...
            cwd=CLIENTS / "rust",
            requires="cargo",
            note="nexus-graph-sdk RPC transport, thunder-rpc",
        ),
        Cell(
            name="typescript",
        ...
        Cell(
            ...
            note="hivellm-thunder via nexus_sdk",
        ...
        Cell(
            ...
            cwd=CLIENTS / "csharp",
            requires="dotnet",
            note="HiveLLM.Thunder via Nexus.SDK",
        ),
        Cell(
            name="go",
            command=["go", "run", "."],
            cwd=CLIENTS / "go",
            requires="go",
            note="thunder-go via the Go SDK",
        ),
        Cell(
            name="php",
...
def server_binary() -> Path:
    ...
    path = REPO / "target" / "release" / f"nexus-server{exe}"
    if not path.exists():
        raise SystemExit(
            f"server binary not found at {path}\n"
            "build it first: cargo +nightly build --release -p nexus-server"
        )
    return path
...
def server_env(workdir: Path) -> dict[str, str]:
    """Translate server-config.yml into the env vars nexus-server reads.

    nexus-server parses only ``server.addr`` from a config.yml; the RPC / RESP3
    / auth knobs live in env vars, so the matrix is the thing that keeps the
    yml and the boot in sync.
    """
    return {
        **os.environ,
        "NEXUS_CONFIG_PATH": str(workdir / "config.yml"),
        "NEXUS_ADDR": REST_ADDR,
        "NEXUS_DATA_DIR": str(workdir / "data"),
        "NEXUS_RPC_ENABLED": "true",
        "NEXUS_RPC_ADDR": RPC_ADDR,
        "NEXUS_RPC_REQUIRE_AUTH": "true",
        "NEXUS_RESP3_ENABLED": "true",
        "NEXUS_RESP3_ADDR": RESP3_ADDR,
        "NEXUS_RESP3_REQUIRE_AUTH": "true",
        "NEXUS_AUTH_ENABLED": "true",
        "NEXUS_ROOT_PASSWORD": PASSWORD,
    }
...
def run_cell(cell: Cell, timeout: float, target: tuple[str, int]) -> Result:
    # `NEXUS_INTEROP_<CELL>` overrides the launcher for one cell, e.g.
    # `NEXUS_INTEROP_PHP=C:\php\php.exe`. Needed when the name on PATH is not
    # something that can actually be spawned (a winget App Execution Alias).
    override = os.environ.get(f"NEXUS_INTEROP_{cell.name.upper()}")
    if override:
...
def main() -> int:
    ...
    parser.add_argument(
        "--server",
        metavar="HOST:PORT",
        help="measure an already-running server instead of starting one. The "
        "server must be configured like server-config.yml: auth required, same "
        "credentials.",
    )
    ...
    if args.server:
        ...
        with tempfile.TemporaryDirectory(
            prefix="nexus-interop-", ignore_cleanup_errors=True
        ) as tmp:
