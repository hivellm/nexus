...
# In WSL2, the Windows host is typically the default gateway
...
if ($routeOutput -match 'default via (\d+\.\d+\.\d+\.\d+)') {
    ...
    # Fallback: get WSL IP and use .1 as Windows host (common in WSL2)
    $wslIpOutput = wsl -d Ubuntu-24.04 -- bash -l -c "hostname -I"
    $wslIp = ($wslIpOutput.Trim() -split '\s+')[0]
    if ($wslIp -match '^(\d+\.\d+\.\d+)\.\d+$') {
        $windowsHostIp = $matches[1] + ".1"
    } else {
        # Last resort
        $windowsHostIp = "192.168.0.1"
    }
}
