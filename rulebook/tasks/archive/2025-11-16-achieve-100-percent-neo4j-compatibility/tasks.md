...
**Last updated**: 2025-11-16 (Session 4 - CREATE Bug #2 Root Cause Identified! 🔍)
...
3. 🔍 **Phase 6: CREATE Duplication Bug #2 - ROOT CAUSE IDENTIFIED**
   - Symptom: 3 CREATE statements create 4 nodes in storage
   - Debugging Results:
     - Storage assigns IDs: 1 (Alice), 2 (Bob), 3 (Acme)
     - MATCH (n) returns IDs: 0, 1, 2, 3 (4 nodes, offset by -1)
     - Node ID mismatch: Storage ID 1 → MATCH shows as ID 0
   - ROOT CAUSE: ID mapping issue in `read_node_as_value()` or `id()` function
     - Storage layer starts IDs from 1
     - Executor/MATCH returns IDs starting from 0
     - Creates "ghost" node at ID 0
     - All IDs shifted by -1
   - Fix Attempt #1: Added `skip_next_node` flag to prevent duplicate node creation in relationships (PARTIAL)
   - Status: ❌ Need to fix ID mapping in next session
   - Files Modified:
     - `nexus-core/src/executor/mod.rs`: Added `skip_next_node` logic
     - `nexus-core/tests/test_relationship_debug.rs`: Added bug documentation
